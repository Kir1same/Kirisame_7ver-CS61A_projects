...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    temp_string=[x for x in paragraphs if select(x)==True]
    if(k>= len(temp_string) ):
        return ''
    else:
        return temp_string[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    def check_subject_words(test_words):
        test_words = remove_punctuation(test_words)
        test_words = lower(test_words)
        test_split_words=split(test_words)
        for i in test_split_words:
            if i in subject:
                return True
        return False
        
    return check_subject_words
...
def accuracy(typed, source):
    ...
    for i in range(min(len(source_words),len(typed_words))):
        if typed_words[i]==source_words[i]:
            tick+=1
    return tick*100/len(typed_words)
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    word_of_type=len(typed)/5
    return word_of_type*(60/elapsed)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(len(word_list)):
        if mininum_auto>diff_function(typed_word,word_list[i],limit) or (index_ofnum==-1 and mininum_auto==diff_function(typed_word,word_list[i],limit)):
            index_ofnum=i
            mininum_auto=diff_function(typed_word,word_list[i],limit)
    if index_ofnum ==-1:
        return typed_word
    else:
        return word_list[index_ofnum]
...
def time_per_word(words, timestamps_per_player):
    ...
    [[6, 3, 6, 2], [10, 6, 1, 2]]
    """
    output_list=[i for i in range(len(timestamps_per_player))]
    for i in range(len(timestamps_per_player)):
        temp_list=[]
        for j in range(1,len(timestamps_per_player[i])):
            temp_list+=[timestamps_per_player[i][j]-timestamps_per_player[i][j-1]]
        output_list[i]=temp_list
        

    # BEGIN PROBLEM 9
    return words,output_list
    # END PROBLEM 9
...
def time_per_word_match(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    word1,time1=time_per_word(words,timestamps_per_player)
 
    class match_words:
        def __init__(self,word1,time1):
            self.a=word1
            self.b=time1
    ...
    return match_words(word1,time1)
    # END PROBLEM 10
...
def fastest_words(match_object):
    ...
    output_list=[[]*i for i in player_indices]

    times=get_all_times(match_object)
    words=get_all_words(match_object)
    for i in word_indices:
        minimun_temp=times[0][i]
        output_stamp=0
        for j in player_indices:
            if times[j][i]< minimun_temp:
                minimun_temp= times[j][i]
                output_stamp=j
        output_list[output_stamp].append(words[i])
    return output_list
    # END PROBLEM 11
